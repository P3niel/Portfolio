...
const features = [
  ["Kernel Runtime", "Capture déterministe des étapes, observations et évaluations."],
  ["Replay", "Reconstruction d'une timeline immuable à partir d'un run."],
  ["Run Comparison", "Comparaison des durées, étapes, observations et évaluations."],
  ["Anomaly Detection", "Détection des runs lents, bloqués, en erreur ou incohérents."],
  ["Static Dashboard", "Exploration de payloads Kernel embarqués dans une interface statique."],
  ["Governance", "Contrats versionnés, manifests et validation d'intégration."],
];
...
export default function AiObsPage() {
    ...
    <main className="min-h-screen px-6 py-20">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <header className="mb-5 border border-rule bg-surface/40 p-6 md:p-8">
          ...
          <p className="max-w-4xl text-sm leading-7 text-ink-2">AI-Obs est un prototype gouverné conçu pour rendre les exécutions d’agents IA inspectables, comparables et reproductibles. Son Kernel enregistre les faits d’exécution, puis des modules dédiés produisent analyses, alertes, replay et comparaisons.</p>
        ...
        <section className="mb-5 grid gap-5 border border-rule bg-surface/20 p-6 lg:grid-cols-[0.85fr_1.15fr]">
          <div>
            ...
            <p className="mb-4 text-sm leading-6 text-ink-2">Les systèmes IA deviennent plus autonomes, mais leur fonctionnement reste souvent opaque. AI-Obs transpose les principes des systèmes distribués — logs, traces et métriques — aux agents IA.</p>
            <ul className="grid grid-cols-2 gap-2 text-xs text-ink-2">{["Observer chaque exécution", "Reconstruire le raisonnement opérationnel", "Mesurer les performances", "Détecter les anomalies", "Faciliter le debugging", "Fournir une base de gouvernance"].map((item) => <li key={item} className="border-l border-accent/50 pl-3">{item}</li>)}</ul>
          </div>
          <div>
        ...
        <section className="mb-5 border border-rule bg-surface/20 p-6">
          <SectionTitle index="03">Fonctionnalités</SectionTitle>
        ...
        <section className="mb-5 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="border border-rule bg-surface/20 p-6">
            <SectionTitle index="04">Exemple d’un run</SectionTitle>
  ...
  "status": "success"
        ...
        <section className="mb-5 border border-rule bg-surface/20 p-6">
          <SectionTitle index="06">Métriques observées</SectionTitle>
        ...
        <section className="mb-5 grid gap-5 lg:grid-cols-2">
          <div className="border border-rule bg-surface/20 p-6"><SectionTitle index="07">Gouvernance</SectionTitle><div className="grid gap-2 sm:grid-cols-2">{governance.map((artifact) => <div key={artifact} className="border-l border-accent/50 px-3 py-2 font-mono text-xs text-ink-2">{artifact}</div>)}</div></div>
        ...
        <section className="mb-5 border border-rule bg-surface/20 p-6 md:p-8">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div>
              <p className="mb-5 max-w-3xl text-sm leading-7 text-ink-2">AI-Obs est développé selon un processus structuré inspiré des pratiques d’ingénierie logicielle modernes. Chaque évolution est planifiée, documentée et validée avant son intégration.</p>
              <div className="grid gap-4 sm:grid-cols-2">
                <article className="border border-rule bg-bg/30 p-4">
                  <h3 className="mb-3 font-mono text-xs uppercase tracking-[0.14em] text-accent">Planification</h3>
                  <ul className="space-y-2 text-xs leading-5 text-ink-2">{["Product backlog", "Priorisation des fonctionnalités", "Découpage en tâches techniques", "Objectifs définis pour chaque cycle"].map((item) => <li key={item} className="border-l border-accent/50 pl-3">{item}</li>)}</ul>
                </article>
                <article className="border border-rule bg-bg/30 p-4">
                  <h3 className="mb-3 font-mono text-xs uppercase tracking-[0.14em] text-accent">Ticketing</h3>
                  <p className="mb-3 text-xs leading-5 text-ink-2">Chaque évolution est suivie par un ticket dédié afin d’assurer la traçabilité des décisions et une progression maîtrisée.</p>
                  <p className="font-mono text-[10px] leading-5 text-ink-3">OBJECTIF · PÉRIMÈTRE · CRITÈRES D’ACCEPTATION · LIVRABLES · STATUT</p>
                </article>
                <article className="border border-rule bg-bg/30 p-4">
                  <h3 className="mb-3 font-mono text-xs uppercase tracking-[0.14em] text-accent">Architecture & Documentation</h3>
                  <p className="mb-3 text-xs leading-5 text-ink-2">La conception est documentée avant toute implémentation importante.</p>
                ...
                <article className="border border-rule bg-bg/30 p-4">
                  ...
                  <p className="text-xs leading-5 text-ink-2">Chaque contribution suit un processus de revue et de validation avant intégration afin de préserver la cohérence de l’architecture et les conventions du projet.</p>
                </article>
              </div>
            </div>
            ...
            <aside className="border border-accent/40 bg-accent/5 p-5">
              ...
              <p className="mt-4 text-[10px] leading-5 text-ink-3">Chiffres issus des artefacts actuellement versionnés dans le repository.</p>
            </aside>
          </div>
        ...
        <section className="mb-5 border border-rule bg-surface/20 p-6"><SectionTitle index="10">Ce que j’apprends</SectionTitle><div className="flex flex-wrap gap-2">{learnings.map((item) => <span key={item} className="border border-rule px-3 py-1.5 text-xs text-ink-2">{item}</span>)}</div></section>

        <footer className="border border-accent/40 bg-accent/5 p-6 md:p-8"><p className="max-w-4xl text-sm leading-7 text-ink-2">AI-Obs est né d’une question simple : comment comprendre précisément ce qu’un système d’IA fait pendant son exécution ? Le projet applique les principes de l’observabilité aux systèmes IA afin de fournir des traces exploitables, des métriques fiables et une base solide pour le debugging, l’audit et la gouvernance.</p><a href="https://github.com/P3niel/ai-observability" target="_blank" rel="noopener noreferrer" className="mt-5 inline-block border border-accent px-4 py-2 font-mono text-xs text-accent transition-colors hover:bg-accent hover:text-bg">Voir le repository ↗</a></footer>
      </div>
    </main>
  );
}
